...
app.post('/Adm/Admin_Info', verifyBearerToken, async (req, res) => {
    ...
    try {
        conn = await pool.getConnection();
        const query = 'Select Adm_Id, Adm_Name From NKSSA_Manager Where Adm_Id = ? And Adm_Name = ? ';
...
app.post('/Adm/Adm_Insert', async (req, res) => {
    ...
    try {
        conn = await pool.getConnection();
        const query = 'Insert Into NKSSA_Manager (Adm_Id, Adm_Pw, Adm_Name) Values (?, ?, ?)';
...
app.post('/Adm/Contets_List', async (req, res) => {
    ...
    try {
        conn = await pool.getConnection();
        const query = 'Select Contents From NKSSA_Contents Where Contents_Type = ? ';
...
app.post('/Adm/Contets_Insert', verifyBearerToken, async (req, res) => {
    ...
    try {
        conn = await pool.getConnection();
        const query = 'Insert Into NKSSA_Contents (Contents_Type, Contents) ' + 'Values ' + '(?, ?)';
...
app.post('/Adm/Contets_Update', verifyBearerToken, async (req, res) => {
    ...
    try {
        conn = await pool.getConnection();
        const query = 'Update NKSSA_Contents set Contents_Type = ?, Contents = ? ' + 'Where Idx = ? ';
...
app.post('/Adm/Calender_Insert', verifyBearerToken, async (req, res) => {
    const { Edu_Nm, Edu_Type, Base_Line, Edu_Date_Start, Edu_Date_End, Edu_Personnel } = req.body;
    let conn;
    try {
        conn = await pool.getConnection();
        const query =
    ...
    try {
        ...
        res.json({
            RET_DATA: result.map((row) => ({
                ...
                Edu_Date_SM: (1 + row.Edu_Date_Start.getMonth()).toString().padStart(2, '0'),
                Edu_Date_SD: row.Edu_Date_Start.getDate().toString().padStart(2, '0'),
                Edu_Date_EM: (1 + row.Edu_Date_End.getMonth()).toString().padStart(2, '0'),
